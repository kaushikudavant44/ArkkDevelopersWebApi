...
public interface GetEmployeeReportDetailsRepository extends JpaRepository<GetEmployeeReportDetails, Integer>{
	...
	@Query(value="SELECT d.DeviceLogId as device_log_id, d.UserId as user_id, e.name, b.name as branch, DATE(d.LogDate)as date, COALESCE((SELECT TIME(dd.LogDate) FROM DeviceLogs dd WHERE"
			...
			+ " d.UserId=e.emp_id AND d.UserId=:userId AND d.Direction='out' AND d.LogDate BETWEEN :fromDate AND :toDate AND d.DeviceId=b.device_id GROUP by d.LogDate" ,nativeQuery=true)
